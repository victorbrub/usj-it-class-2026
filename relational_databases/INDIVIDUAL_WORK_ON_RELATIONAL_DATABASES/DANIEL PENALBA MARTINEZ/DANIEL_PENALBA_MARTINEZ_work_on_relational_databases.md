...
**Student**: Daniel Penalba Martinez
**Model**: Gym and Fitness — Trainers, Members, Classes and Machines
...
The following schedule involves two transactions concurrently accessing the `class` table of the gym model:

- **T1**: The gym manager temporarily assigns a substitute trainer (#8) to class #3 to test the scheduling system. The operation fails mid-way and the transaction is rolled back.
- **T2**: Member #15 reads class #3's trainer information and confirms a booking, seeing the uncommitted substitute trainer.
- Initial value: `class[id_class=3].trainer_id = 2`.

| Step | T1 (Assign substitute trainer — ROLLBACK) | T2 (Member books class)                            |
|------|-------------------------------------------|----------------------------------------------------|
| t1   | BEGIN                                     |                                                    |
| t2   | READ(class[3].trainer_id) = 2             |                                                    |
| t3   | class[3].trainer_id = 8                   |                                                    |
| t4   | WRITE(class[3].trainer_id) = 8            |                                                    |
| t5   |                                           | BEGIN                                              |
| t6   |                                           | READ(class[3].trainer_id) = 8 (DIRTY READ!)        |
| t7   |                                           | INSERT reservation (id_member=15, id_class=3)      |
| t8   |                                           | COMMIT (booked expecting trainer #8)               |
| t9   | ROLLBACK                                  |                                                    |
| t10  | class[3].trainer_id reverted to 2         |                                                    |

T2 committed a booking based on data that was never permanently written. Member #15 booked the class expecting trainer #8, who is not actually assigned (trainer #2 is).
...
SELECT name, surname, level WHERE age < 30 AND id_trainer < 5
FROM member NATURAL JOIN class
JOIN reservation ON id_member = reservation.id_member;
