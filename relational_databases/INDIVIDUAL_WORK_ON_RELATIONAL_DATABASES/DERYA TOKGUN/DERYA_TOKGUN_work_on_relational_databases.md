...
**Student**: Derya Tokgun
**Model**: Hospital — Departments, Doctors, Patients, Appointments and Medical Devices
...
The following schedule involves two transactions concurrently accessing the `Doctor` table of the hospital model:

- **T1**: A scheduling assistant reads doctor #4's `Specialty` twice: once when checking availability for a new patient referral and once when printing the referral document.
- **T2**: The HR system updates doctor #4's `Specialty` from `'General Practice'` to `'Cardiology'` (the doctor has completed a specialist certification) and commits between T1's two reads.
- Initial value: `Doctor[DoctorID=4].Specialty = 'General Practice'`.

| Step | T1 (Scheduling assistant: referral report)          | T2 (HR: update doctor specialty)                      |
|------|-----------------------------------------------------|-------------------------------------------------------|
| t1   | BEGIN                                               |                                                       |
| t2   | READ(Doctor[4].Specialty) = 'General Practice'      |                                                       |
| t3   |                                                     | BEGIN                                                 |
| t4   |                                                     | WRITE(Doctor[4].Specialty) = 'Cardiology'             |
| t5   |                                                     | COMMIT                                                |
| t6   | READ(Doctor[4].Specialty) = 'Cardiology' (DIFFERENT!)  |                                                    |
| t7   | COMMIT (referral printed with wrong specialty)      |                                                       |

T1 read the same `Specialty` field within the same transaction and obtained different values (`'General Practice'` then `'Cardiology'`) because T2 committed an update between T1's two reads. The referral document was generated with inconsistent data.
...
SELECT FirstName, LastName, Name WHERE Specialty = 'Cardiology' AND DeptID < 5
FROM Doctor NATURAL JOIN Patient
JOIN Department ON DeptID = Doctor.DeptID;
