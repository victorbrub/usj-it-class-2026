...
**Student**: Dario Sarasa Navarro
**Model**: Account and Password Manager — Users, Accounts, Categories and Password History
...
The following schedule involves two transactions concurrently accessing the `account` table of the password manager model:

- **T1**: A password-reuse security checker reads the current password hash of account #12 at the start of its scan, and then reads it again at the end to confirm nothing changed.
- **T2**: The account owner updates the password of account #12 and commits between T1's two reads.
- Initial value: `account[account_id=12].password = 'hash_abc123'`.

| Step | T1 (Password reuse check)                               | T2 (Password update)                            |
|------|---------------------------------------------------------|-------------------------------------------------|
| t1   | BEGIN                                                   |                                                 |
| t2   | READ(account[12].password) = 'hash_abc123'              |                                                 |
| t3   |                                                         | BEGIN                                           |
| t4   |                                                         | READ(account[12].password) = 'hash_abc123'      |
| t5   |                                                         | password = 'hash_xyz789'                        |
| t6   |                                                         | WRITE(account[12].password) = 'hash_xyz789'     |
| t7   |                                                         | COMMIT                                          |
| t8   | READ(account[12].password) = 'hash_xyz789' (DIFFERENT!) |                                                 |
| t9   | COMMIT                                                  |                                                 |

T1 read the same row twice within the same transaction and obtained different values. T2 committed a modification between T1's two reads, making T1's assumption that the password was stable invalid.
...
SELECT first_name, last_name, username WHERE age > 18 AND user_id < 10
FROM user NATURAL JOIN account
JOIN account_category ON account_id = account_category.account_id;
