...
**Student**: Juan Jesus Iznardo Salar
**Model**: Pokemon Relational — Trainers, Pokemon Species, Moves and Pokedex
...
The following schedule involves two transactions concurrently accessing the `trainer_card` table of the Pokemon relational model:

- **T1**: A synchronisation service reads trainer #3's `total_caught` field as part of an achievement check.
- **T2**: Meanwhile, a background process catches a new Pokemon and updates `total_caught` for trainer #3, but the operation fails and is rolled back after T1 has already read the (dirty) value.
- Initial value: `trainer_card[trainer_id=3].total_caught = 42`.

| Step | T1 (Achievement check)                              | T2 (Catch new Pokemon — ROLLBACK)             |
|------|-----------------------------------------------------|-----------------------------------------------|
| t1   |                                                     | BEGIN                                         |
| t2   |                                                     | WRITE(trainer_card[3].total_caught) = 43      |
| t3   | BEGIN                                               |                                               |
| t4   | READ(trainer_card[3].total_caught) = 43 (DIRTY!)    |                                               |
| t5   | achievement unlocked: "Caught 43 Pokemon"           |                                               |
| t6   | COMMIT                                              |                                               |
| t7   |                                                     | ROLLBACK                                      |
| t8   |                                                     | trainer_card[3].total_caught reverted to 42   |

T1 committed a decision based on dirty data. The achievement was unlocked for a catch count that was never actually saved.
...
SELECT trainer_name, species_name, total_caught WHERE trainer_id < 5 AND base_hp > 50
FROM trainer NATURAL JOIN trainer_card
JOIN pokemon_species ON species_id = pokemon_species.species_id;
