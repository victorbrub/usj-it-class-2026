...
**Student**: Daniel Escartin Marcen
**Model**: Running Races — Tracks, Races, Persons and Participations
...
The following schedule involves two transactions concurrently writing the `participations` table of the running races model:

- **T1**: A race organizer manually enters the finish time for participation record #42 (the automatic system failed).
- **T2**: The automatic timing chip system also writes the finish time for the same participation record #42 as it processes a delayed data upload.
- Initial value: `participations[id=42].finish_time = NULL`.

| Step | T1 (Manual entry: '02:34:15')                          | T2 (Timing chip: '02:34:10')                        |
|------|--------------------------------------------------------|-----------------------------------------------------|
| t1   | BEGIN                                                  |                                                     |
| t2   | READ(participations[42].finish_time) = NULL            |                                                     |
| t3   |                                                        | BEGIN                                               |
| t4   |                                                        | READ(participations[42].finish_time) = NULL         |
| t5   | finish_time = '02:34:15'                               |                                                     |
| t6   | WRITE(participations[42].finish_time) = '02:34:15'     |                                                     |
| t7   | COMMIT                                                 |                                                     |
| t8   |                                                        | finish_time = '02:34:10'                            |
| t9   |                                                        | WRITE(participations[42].finish_time) = '02:34:10'  |
| t10  |                                                        | COMMIT                                              |

Final value: '02:34:10'. T1's manually entered time ('02:34:15') is overwritten and lost. The correct time to preserve (whichever is authoritative) is discarded.
...
SELECT name, surname, finish_time WHERE finish_position < 10 AND track_id < 5
FROM persons NATURAL JOIN participations
