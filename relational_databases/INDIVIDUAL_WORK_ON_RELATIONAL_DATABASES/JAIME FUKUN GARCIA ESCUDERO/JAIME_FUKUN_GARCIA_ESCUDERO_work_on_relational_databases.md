...
**Student**: Jaime Fukun Garcia Escudero
**Model**: Sports Club — Coaches, Partners, Classes and Memberships
...
The following schedule involves two transactions concurrently modifying the `membership` table of the sports club model:

- **T1**: The club admin renews partner #9's membership, updating the `end_date` by one year.
- **T2**: The billing system simultaneously processes a price adjustment (5% increase) for partner #9's membership. Both read the same current `price` and write independently.
- Initial value: `membership[id_partner=9].price = 50.00`.

| Step | T1 (Admin: renews membership)                  | T2 (Billing: price increase)                  |
|------|------------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                          |                                               |
| t2   | READ(membership[9].price) = 50.00              |                                               |
| t3   |                                                | BEGIN                                         |
| t4   |                                                | READ(membership[9].price) = 50.00             |
| t5   | WRITE(membership[9].end_date = new_date)       |                                               |
| t6   | WRITE(membership[9].price) = 50.00 (unchanged) |                                               |
| t7   | COMMIT                                         |                                               |
| t8   |                                                | price = 50.00 * 1.05 = 52.50                  |
| t9   |                                                | WRITE(membership[9].price) = 52.50            |
| t10  |                                                | COMMIT                                        |

In this case there is no lost update since T1 only changed the date and T2 only changed the price. Discuss whether the schedule is serializable or not.
...
SELECT name, class_name, horary WHERE age < 40 AND id_coach < 3
FROM partner NATURAL JOIN attends
JOIN class ON id_coach = class.id_coach;
