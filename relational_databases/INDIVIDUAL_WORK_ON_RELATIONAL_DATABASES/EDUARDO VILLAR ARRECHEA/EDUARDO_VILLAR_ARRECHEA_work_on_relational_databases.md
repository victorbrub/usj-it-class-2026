...
**Model**: MOBA Game — Heroes, Items, Players, Builds and Matches
...
The following schedule involves two transactions concurrently accessing the `build` table of the MOBA game model:

- **T1**: A game balance analyst reads build #2's `total_cost` twice within the same statistical report.
- **T2**: The shop module adds a new item to build #2, which triggers an update to `total_cost` via trigger, committing between T1's two reads.
- Initial value: `build[build_id=2].total_cost = 6500.00`.

| Step | T1 (Balance analysis report)                           | T2 (Player adds item to build)               |
|------|--------------------------------------------------------|----------------------------------------------|
| t1   | BEGIN                                                  |                                              |
| t2   | READ(build[2].total_cost) = 6500.00                    |                                              |
| t3   |                                                        | BEGIN                                        |
| t4   |                                                        | INSERT build_item (build_id=2, item_id=7)    |
| t5   |                                                        | TRIGGER updates build[2].total_cost = 8200.00|
| t6   |                                                        | COMMIT                                       |
| t7   | READ(build[2].total_cost) = 8200.00 (DIFFERENT VALUE!) |                                              |
| t8   | COMMIT                                                 |                                              |

T1 read the same derived field twice and obtained different values (6500.00, then 8200.00) because T2 committed a change that updated the derived attribute between T1's reads.
...
SELECT name, build_name, total_cost WHERE base_health > 500 AND id_player < 10
FROM hero NATURAL JOIN build
JOIN player ON id_player = player.id_player;
