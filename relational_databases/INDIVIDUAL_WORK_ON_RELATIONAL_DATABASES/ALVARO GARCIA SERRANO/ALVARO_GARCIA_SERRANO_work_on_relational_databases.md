...
**Student**: Alvaro Garcia Serrano
**Model**: E-Commerce — Users, Products, Orders and Categories
...
The following schedule involves two transactions concurrently accessing the `products` table of the e-commerce model:

- **T1**: An inventory report generator runs a query counting all products in category #2 (Electronics) at the beginning and end of a report generation cycle.
- **T2**: A product manager inserts 5 new electronics products (category_id=2) during the time T1 is running.
- Initial state: 50 products with category_id = 2.

| Step | T1 (Inventory report)                                  | T2 (Add new electronics products)         |
|------|--------------------------------------------------------|-------------------------------------------|
| t1   | BEGIN                                                  |                                           |
| t2   | COUNT(products WHERE category_id=2) = 50               |                                           |
| t3   |                                                        | BEGIN                                     |
| t4   |                                                        | INSERT 5 new products (category_id=2)     |
| t5   |                                                        | COMMIT                                    |
| t6   | COUNT(products WHERE category_id=2) = 55 (DIFFERENT!)  |                                           |
| t7   | COMMIT                                                 |                                           |

T1 saw a different set of rows in its two reads within the same transaction. The 5 new rows inserted and committed by T2 are visible to T1's second count.
...
SELECT name, price WHERE stock > 0 AND category_id < 3
