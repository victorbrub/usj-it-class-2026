...
**Student**: Pablo Ruslan Tudela Rubio
**Model**: Veterinary Clinic — Owners, Animals, Veterinarians, Treatments and Vaccines
...
The following schedule involves two transactions concurrently accessing the `treatment` table of the veterinary clinic model:

- **T1**: The billing system reads treatment #8's `cost` twice: once when generating an estimate and once when issuing the final invoice.
- **T2**: The veterinarian updates treatment #8's `cost` from 45.00 to 65.00 (additional procedures added) and commits between T1's two reads.
- Initial value: `treatment[treatment_id=8].cost = 45.00`.

| Step | T1 (Billing: generate estimate and invoice)         | T2 (Vet: update treatment cost)               |
|------|-----------------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                               |                                               |
| t2   | READ(treatment[8].cost) = 45.00 (estimate)          |                                               |
| t3   |                                                     | BEGIN                                         |
| t4   |                                                     | WRITE(treatment[8].cost) = 65.00              |
| t5   |                                                     | COMMIT                                        |
| t6   | READ(treatment[8].cost) = 65.00 (DIFFERENT VALUE!)  |                                               |
| t7   | COMMIT (invoice uses 65.00 but estimate used 45.00) |                                               |

T1 read the same cost within the same transaction and obtained different values (45.00 then 65.00) because T2 committed an update between T1's two reads. The estimate and final invoice disagree.
...
SELECT full_name, name, cost WHERE cost > 30 AND owner_id < 5
FROM owner NATURAL JOIN animal
JOIN treatment ON vet_id = treatment.vet_id;
