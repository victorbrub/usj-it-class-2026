...
**Student**: Burak Enoz
**Model**: Marine Biology — Deep-Sea Creatures, Habitats, Expeditions and Observations
...
The following schedule involves two transactions concurrently accessing the `Creatures` table of the marine biology model:

- **T1**: A research system reads creature #7's maximum diving depth (`max_depth`) twice during a depth-range analysis report.
- **T2**: A field researcher corrects a measurement error, updating creature #7's `max_depth` from 600 to 450 meters, and commits between T1's two reads.
- Initial value: `Creatures[creature_id=7].max_depth = 600`.

| Step | T1 (Depth analysis report)                              | T2 (Measurement correction)                  |
|------|----------------------------------------------------------|----------------------------------------------|
| t1   | BEGIN                                                    |                                              |
| t2   | READ(Creatures[7].max_depth) = 600                       |                                              |
| t3   |                                                          | BEGIN                                        |
| t4   |                                                          | READ(Creatures[7].max_depth) = 600           |
| t5   |                                                          | max_depth = 450 (corrected)                  |
| t6   |                                                          | WRITE(Creatures[7].max_depth) = 450          |
| t7   |                                                          | COMMIT                                       |
| t8   | READ(Creatures[7].max_depth) = 450 (DIFFERENT VALUE!)    |                                              |
| t9   | COMMIT                                                   |                                              |

T1 read the same row twice within the same transaction and obtained different values (600, then 450) because T2 committed a modification between T1's two reads.
...
SELECT scientific_name, common_name WHERE habitat_id = 3 AND min_depth < 200
FROM Creatures NATURAL JOIN Expeditions
JOIN Observations ON creature_id = Observations.creature_id;
