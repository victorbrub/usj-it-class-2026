...
**Model**: RPG Spell/Combat — Players, Enemies, Spells and Modified Spells
...
The following schedule involves two transactions concurrently accessing the `enemy` table of the RPG combat model:

- **T1**: A combat log display reads enemy #3's `actual_hp` to display the current health bar on screen.
- **T2**: A spell resolution system applies damage to enemy #3's `actual_hp`, subtracting 30 HP. The operation fails (the spell is interrupted) and the transaction is rolled back. T1 reads the HP during T2's active write.
- Initial value: `enemy[enemy_id=3].actual_hp = 120`.

| Step | T1 (Display health bar)                              | T2 (Spell deals 30 damage — ROLLBACK)              |
|------|------------------------------------------------------|----------------------------------------------------|
| t1   |                                                      | BEGIN                                              |
| t2   |                                                      | WRITE(enemy[3].actual_hp) = 90 (120 - 30)         |
| t3   | BEGIN                                                |                                                    |
| t4   | READ(enemy[3].actual_hp) = 90 (DIRTY READ!)          |                                                    |
| t5   | COMMIT (display shows 90 HP)                         |                                                    |
| t6   |                                                      | ROLLBACK                                           |
| t7   |                                                      | enemy[3].actual_hp reverted to 120                |

T1 committed a read based on data written by a transaction that was never committed. The player saw incorrect HP data for the enemy.
...
SELECT mod_name, mod_damage, level WHERE mod_damage > 10 AND enemy_id < 5
FROM enemy NATURAL JOIN enemy_mod_spell
JOIN mod_spell ON spell_id = mod_spell.spell_id;
