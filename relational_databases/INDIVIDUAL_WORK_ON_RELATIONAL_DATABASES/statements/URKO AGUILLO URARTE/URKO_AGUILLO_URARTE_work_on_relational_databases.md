...
**Student**: Urko Aguillo Urarte
**Model**: RPG / Board Game Shop — Categories, Items, Users, Orders and Order Items
...
The following schedule involves two transactions concurrently accessing the `items` table of the board game shop model:

- **T1**: A stock auditor counts the number of in-stock dice products (category 'dice') twice: at the start and end of a stock summary report.
- **T2**: The warehouse management system restocks the shop by inserting a new dice item into the `items` table and commits between T1's two counts.
- Initial state: 8 dice items with `stock > 0`.

| Step | T1 (Stock audit report: count dice in stock)                        | T2 (Warehouse: add new dice item)                       |
|------|---------------------------------------------------------------------|----------------------------------------------------------|
| t1   | BEGIN                                                               |                                                          |
| t2   | SELECT COUNT(*) FROM items WHERE category_id=2 AND stock>0 = 8      |                                                          |
| t3   |                                                                     | BEGIN                                                    |
| t4   |                                                                     | INSERT INTO items (category_id=2, stock=10, name='D20')  |
| t5   |                                                                     | COMMIT                                                   |
| t6   | SELECT COUNT(*) FROM items WHERE category_id=2 AND stock>0 = 9 (!!) |                                                          |
| t7   | COMMIT (report shows inconsistent counts: 8 then 9)                 |                                                          |

T1 counted the same set of rows within the same transaction and obtained different values (8 then 9) because T2 inserted a new item row between T1's two COUNT queries.
...
SELECT username, name, quantity WHERE order_date > '2024-01-01' AND user_id < 5
FROM users NATURAL JOIN orders
JOIN order_item ON item_id = order_item.item_id;
