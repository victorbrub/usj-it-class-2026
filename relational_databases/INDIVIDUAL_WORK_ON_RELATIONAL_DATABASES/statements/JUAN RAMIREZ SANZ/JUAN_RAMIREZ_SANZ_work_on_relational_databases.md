...
**Student**: Juan Ramirez Sanz
**Model**: Hotel — Guests, Employees, Rooms, Reservations and Invoices
...
The following schedule involves two transactions concurrently accessing the `room` table of the hotel model:

- **T1**: Revenue manager session A reads room #8's price to apply a seasonal discount and writes the discounted price.
- **T2**: Revenue manager session B simultaneously reads room #8's price to apply a weekend surcharge and writes the markup. Both start from the same value, last one to write wins.
- Initial value: `room[room_id=8].price = 100.00`.

| Step | T1 (Seasonal discount: -10%)                  | T2 (Weekend surcharge: +20%)                  |
|------|-----------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                         |                                               |
| t2   | READ(room[8].price) = 100.00                  |                                               |
| t3   |                                               | BEGIN                                         |
| t4   |                                               | READ(room[8].price) = 100.00                  |
| t5   | price = 100.00 * 0.90 = 90.00                 |                                               |
| t6   | WRITE(room[8].price) = 90.00                  |                                               |
| t7   | COMMIT                                        |                                               |
| t8   |                                               | price = 100.00 * 1.20 = 120.00 (stale read!)  |
| t9   |                                               | WRITE(room[8].price) = 120.00                 |
| t10  |                                               | COMMIT                                        |

Final price is 120.00 instead of the intended 108.00 (90.00 * 1.20). T1's discount is entirely lost. The room is overpriced because T2 never saw T1's committed update.
...
SELECT name, room_number, total_price WHERE check_in > '2024-01-01' AND id_client < 10
FROM client NATURAL JOIN reservation
JOIN room ON room_id = room.room_id;
