...
**Student**: Julio Estevez Perez
**Model**: Music Concerts — Artists, Venues, Concerts, Posters and Performances
...
The following schedule involves two transactions concurrently accessing the `CONCERT` table of the music concerts model:

- **T1**: A ticket booking agent reads concert #5's `ticket_price` at the start of a purchase and again before finalizing to confirm the price hasn't changed.
- **T2**: The promoter applies a last-minute VIP price surge, updating concert #5's `ticket_price` from 60.00 to 85.00 and committing between T1's two reads.
- Initial value: `CONCERT[concert_id=5].ticket_price = 60.00`.

| Step | T1 (Booking agent: confirm price)                     | T2 (Promoter: apply price surge)               |
|------|-------------------------------------------------------|------------------------------------------------|
| t1   | BEGIN                                                 |                                                |
| t2   | READ(CONCERT[5].ticket_price) = 60.00                 |                                                |
| t3   |                                                       | BEGIN                                          |
| t4   |                                                       | WRITE(CONCERT[5].ticket_price) = 85.00         |
| t5   |                                                       | COMMIT                                         |
| t6   | READ(CONCERT[5].ticket_price) = 85.00 (DIFFERENT!)    |                                                |
| t7   | COMMIT (purchase abandoned — price changed)           |                                                |

T1 read the same price within the same transaction and obtained two different values (60.00 and 85.00) because T2 committed a price change between T1's reads.
...
SELECT artist_name, concert_date, ticket_price WHERE venue_id < 5 AND ticket_price > 30
FROM ARTIST NATURAL JOIN PERFORMANCE
JOIN CONCERT ON venue_id = CONCERT.venue_id;
