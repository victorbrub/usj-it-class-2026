...
**Student**: Gines Gonzalez Brusca
**Model**: Social Network — Users, Friendships, Posts, Stories and Comments
...
The following schedule involves two transactions concurrently accessing the `post` table of the social network model:

- **T1**: A content moderation system marks post #7 for review by temporarily setting its description to '[UNDER REVIEW]'. The moderation action fails and the transaction is rolled back.
- **T2**: Another user reads post #7's description to display it on their feed, reading while T1 is still active.
- Initial value: `post[post_id=7].description = 'Check out my new photo!'`.

| Step | T1 (Moderate post — ROLLBACK)                            | T2 (Read post for feed)                                      |
|------|----------------------------------------------------------|--------------------------------------------------------------|
| t1   | BEGIN                                                    |                                                              |
| t2   | WRITE(post[7].description) = '[UNDER REVIEW]'            |                                                              |
| t3   |                                                          | BEGIN                                                        |
| t4   |                                                          | READ(post[7].description) = '[UNDER REVIEW]' (DIRTY READ!)   |
| t5   |                                                          | COMMIT (feed shows '[UNDER REVIEW]')                         |
| t6   | ROLLBACK                                                 |                                                              |
| t7   | post[7].description reverted to 'Check out my new photo!'|                                                              |

T2 committed a read based on data that was never permanently written. A user's feed displayed incorrect moderation text for a post that was not actually under review.
...
SELECT first_name, last_name, description WHERE age > 18 AND user_id < 10
FROM user NATURAL JOIN post
JOIN comment ON user_id = comment.user_id;
