...
**Student**: Eduardo Sandu Zaharia
**Model**: E-Commerce Cart and Orders — Users, Carts, Items and Orders
...
The following schedule involves two transactions concurrently accessing the `cart` table of the e-commerce cart model:

- **T1**: A pricing update process reads cart #3's total twice: once at the start and once at the end of the recalculation report to verify consistency.
- **T2**: A background coupon system applies a discount, updating cart #3's total from 120.00 to 96.00 and committing between T1's two reads.
- Initial value: `cart[cart_id=3].total = 120.00`.

| Step | T1 (Price consistency check)                    | T2 (Apply 20% coupon discount)            |
|------|--------------------------------------------------|-------------------------------------------|
| t1   | BEGIN                                            |                                           |
| t2   | READ(cart[3].total) = 120.00                     |                                           |
| t3   |                                                  | BEGIN                                     |
| t4   |                                                  | READ(cart[3].total) = 120.00              |
| t5   |                                                  | total = 120.00 * 0.80 = 96.00             |
| t6   |                                                  | WRITE(cart[3].total) = 96.00              |
| t7   |                                                  | COMMIT                                    |
| t8   | READ(cart[3].total) = 96.00 (DIFFERENT VALUE!)   |                                           |
| t9   | COMMIT                                           |                                           |

T1 read the same field twice within the same transaction and obtained different values (120.00, then 96.00), because T2 committed a modification between T1's two reads.
...
SELECT name, item_name, quantity WHERE price > 10 AND user_id < 20
FROM user NATURAL JOIN cart_item
JOIN item ON item_id = item.item_id;
