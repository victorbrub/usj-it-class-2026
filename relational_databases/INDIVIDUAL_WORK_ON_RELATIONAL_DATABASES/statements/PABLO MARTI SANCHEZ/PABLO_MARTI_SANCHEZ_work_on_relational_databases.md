...
**Student**: Pablo Marti Sanchez
**Model**: Sports / Olympics — Countries, Stadiums, Sports, Athletes and Medals
...
The following schedule involves two transactions concurrently inserting new rows into the `medal` table of the sports/Olympics model:

- **T1**: A medal tally report counts the total number of medals won by athletes from country #3. It performs the count twice: once at the start and once at the end of a ranking report generation.
- **T2**: A results system inserts a new gold medal for an athlete from country #3 and commits between T1's two counts.
- Initial state: 7 medals for country #3.

| Step | T1 (Ranking report: count medals)                          | T2 (Results: award new medal)                          |
|------|-----------------------------------------------------------|--------------------------------------------------------|
| t1   | BEGIN                                                     |                                                        |
| t2   | SELECT COUNT(*) FROM medal JOIN athlete ... country=3 = 7 |                                                        |
| t3   |                                                           | BEGIN                                                  |
| t4   |                                                           | INSERT INTO medal (type='Gold', athlete_dni='12345678A')|
| t5   |                                                           | COMMIT                                                 |
| t6   | SELECT COUNT(*) FROM medal JOIN athlete ... country=3 = 8 |                                                        |
| t7   | COMMIT (ranking shows inconsistent totals: 7 then 8)      |                                                        |

T1 read the same medal count within the same transaction and obtained different values (7 then 8) because T2 inserted a new row between T1's two COUNT queries.
...
SELECT name, nombre, type WHERE age < 30 AND country_id < 5
FROM athlete NATURAL JOIN athlete_sport
JOIN sport ON stadium_id = sport.stadium_id;
