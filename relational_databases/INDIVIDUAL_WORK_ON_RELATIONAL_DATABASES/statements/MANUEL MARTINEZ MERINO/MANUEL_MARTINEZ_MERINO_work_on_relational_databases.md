...
**Student**: Manuel Martinez Merino
**Model**: Hotel — Guests, Employees, Services, Rooms, Reservations and Invoices
...
The following schedule involves two transactions concurrently inserting new rows into the `reservation` table of the hotel model:

- **T1**: The front desk checks the number of active reservations for room #5 in December before approving a new booking.
- **T2**: An online booking system inserts a new reservation for room #5 in December while T1 is counting, committing between T1's two counts.
- Initial state: 2 active reservations for room #5 in December.

| Step | T1 (Front desk: check availability)                            | T2 (Online booking: new reservation)                      |
|------|----------------------------------------------------------------|-----------------------------------------------------------|
| t1   | BEGIN                                                          |                                                           |
| t2   | SELECT COUNT(*) FROM reservation WHERE num_room=5 ... = 2     |                                                           |
| t3   |                                                                | BEGIN                                                     |
| t4   |                                                                | INSERT INTO reservation (num_room=5, check_in='2024-12-20')|
| t5   |                                                                | COMMIT                                                    |
| t6   | SELECT COUNT(*) FROM reservation WHERE num_room=5 ... = 3 (!!) |                                                           |
| t7   | COMMIT (inconsistent: count changed within the same transaction)|                                                           |

T1's second count returns 3 instead of 2 because T2 inserted a new row between T1's two COUNT queries. The new row was not present in the first read but appeared in the second.
...
SELECT name, surnames, n_noches WHERE check_in_date > '2024-01-01' AND client_dni < 10
FROM client NATURAL JOIN reservation
JOIN room ON num_room = room.num_room;
