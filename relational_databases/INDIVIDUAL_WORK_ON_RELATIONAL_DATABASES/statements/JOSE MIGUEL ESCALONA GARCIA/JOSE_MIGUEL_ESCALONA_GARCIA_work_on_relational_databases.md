...
**Student**: Jose Miguel Escalona Garcia
**Model**: Card Collection — Owners, Collections, Rareness and Cards
...
The following schedule involves two transactions concurrently accessing the `card` table of the card collection model:

- **T1**: A collector reads card #5's market price at the beginning of a purchase transaction and again at the end to confirm it hasn't changed before committing.
- **T2**: The marketplace admin updates card #5's price from 25.00 to 35.00 and commits between T1's two reads.
- Initial value: `card[card_id=5].price = 25.00`.

| Step | T1 (Collector: confirm purchase price)        | T2 (Admin: update card market price)          |
|------|-----------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                         |                                               |
| t2   | READ(card[5].price) = 25.00                   |                                               |
| t3   |                                               | BEGIN                                         |
| t4   |                                               | WRITE(card[5].price) = 35.00                  |
| t5   |                                               | COMMIT                                        |
| t6   | READ(card[5].price) = 35.00 (DIFFERENT VALUE!)|                                               |
| t7   | COMMIT (purchase denied — price changed)      |                                               |

T1 read the same card price twice within the same transaction and obtained different values (25.00 then 35.00) because T2 committed an update between T1's two reads.
...
SELECT owner_name, card_name, rarity_name WHERE card_id < 20 AND price > 10
FROM owner NATURAL JOIN collection
JOIN card ON rareness_id = card.rareness_id;
