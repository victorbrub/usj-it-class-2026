...
**Student**: Javier Nieto Jimenez
**Model**: Pokemon ER — Gyms, Medals, Trainers, and Pokemon
...
The following schedule involves two transactions concurrently accessing the `pokemon` table of the Pokemon model:

- **T1**: A training simulator and a battle system both update Pokemon #12's `level` after their respective events. Both read level=15 and independently compute a new level, creating a lost update.
- Initial value: `pokemon[pokemon_id=12].level = 15`.

| Step | T1 (Training simulator)                       | T2 (Battle reward system)                      |
|------|-----------------------------------------------|------------------------------------------------|
| t1   | BEGIN                                         |                                                |
| t2   | READ(pokemon[12].level) = 15                  |                                                |
| t3   |                                               | BEGIN                                          |
| t4   |                                               | READ(pokemon[12].level) = 15                   |
| t5   | level = 15 + 2 = 17 (training bonus)          |                                                |
| t6   | WRITE(pokemon[12].level) = 17                 |                                                |
| t7   | COMMIT                                        |                                                |
| t8   |                                               | level = 15 + 1 = 16 (battle reward — stale!)   |
| t9   |                                               | WRITE(pokemon[12].level) = 16                  |
| t10  |                                               | COMMIT                                         |

The final level is 16 instead of 18. T1's +2 training bonus is completely overwritten by T2's stale write. Both transactions committed successfully, but the training gain is lost.
...
SELECT trainer_name, pokemon_name, level WHERE trainer_id < 5 AND level > 10
FROM trainer NATURAL JOIN pokemon
JOIN gym ON badge_count = gym.badge_count;
