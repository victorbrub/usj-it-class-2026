...
**Model**: Gym / Fitness Club — Members, Trainers, Classes, Payments and Enrolments
...
The following schedule involves two transactions concurrently accessing the `Class` table of the gym/fitness club model:

- **T1**: The online registration portal reads class #6's `Capacity` to check whether a new member can book a spot.
- **T2**: The gym reception simultaneously registers a new enrolment for class #6 by checking the same `Capacity` value and reducing it by 1. Both T1 and T2 read the same stale capacity before either writes back.
- Initial value: `Class[ClassID=6].Capacity = 1` (last available spot).

| Step | T1 (Online portal: check availability)         | T2 (Reception: register enrolment)            |
|------|------------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                          |                                               |
| t2   | READ(Class[6].Capacity) = 1                    |                                               |
| t3   |                                                | BEGIN                                         |
| t4   |                                                | READ(Class[6].Capacity) = 1                   |
| t5   | Capacity = 1 - 1 = 0                           |                                               |
| t6   | WRITE(Class[6].Capacity) = 0                   |                                               |
| t7   | COMMIT                                         |                                               |
| t8   |                                                | Capacity = 1 - 1 = 0 (stale read!)            |
| t9   |                                                | WRITE(Class[6].Capacity) = 0                  |
| t10  |                                                | COMMIT                                        |

Final capacity: 0 in both cases. The class accepted two enrolments but only decremented the capacity once. One member has been registered without consuming a real spot.
...
SELECT Name, ClassName, EnrollDate WHERE EnrollDate > '2024-01-01' AND MemberID < 5
FROM member NATURAL JOIN enrolls
JOIN class ON TrainerID = class.TrainerID;
