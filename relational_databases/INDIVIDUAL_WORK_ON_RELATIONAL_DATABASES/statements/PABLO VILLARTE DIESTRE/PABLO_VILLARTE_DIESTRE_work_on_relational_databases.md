...
**Student**: Pablo Villarte Diestre
**Model**: Car Database — Manufacturers, Models, Generations, Trims and Power Units
...
The following schedule involves two transactions concurrently inserting new rows into the `TRIM` table of the car database model:

- **T1**: A database analyst counts the number of available trims for generation #4 (a specific BMW 3 Series generation) at the start and end of a catalogue report to verify the count is consistent.
- **T2**: A data entry operator inserts a new trim variant for generation #4 and commits between T1's two counts.
- Initial state: 3 trims for generation #4.

| Step | T1 (Catalogue report: count trims)                          | T2 (Data entry: add new trim variant)                |
|------|-------------------------------------------------------------|------------------------------------------------------|
| t1   | BEGIN                                                       |                                                      |
| t2   | SELECT COUNT(*) FROM TRIM WHERE GENERATION_ID=4 = 3         |                                                      |
| t3   |                                                             | BEGIN                                                |
| t4   |                                                             | INSERT INTO TRIM (GENERATION_ID=4, DRIVETRAIN='AWD') |
| t5   |                                                             | COMMIT                                               |
| t6   | SELECT COUNT(*) FROM TRIM WHERE GENERATION_ID=4 = 4 (!!)    |                                                      |
| t7   | COMMIT (report shows inconsistent counts: 3 then 4)         |                                                      |

T1 counted the same set of rows within the same transaction and obtained different values (3 then 4) because T2 inserted a new TRIM row between T1's two COUNT queries.
...
SELECT name, chassis_code, max_power_kw WHERE max_power_kw > 150 AND manufacturer_id < 5
FROM manufacturer NATURAL JOIN generation
JOIN model ON model_id = model.model_id;
