...
**Student**: Alvaro Echeverria Gay
**Model**: Videogame Store — Products, Orders, Customers and Payments
...
The following schedule involves two transactions concurrently accessing the `product` table of the videogame store model:

- **T1**: A stock manager temporarily sets product #8's stock to 0 to reserve units for a flash sale batch. The operation fails and the transaction is rolled back.
- **T2**: The e-commerce frontend reads the stock of product #8 to show availability to customers. It reads while T1 is still active.
- Initial value: `product_8.stock = 50`.

| Step | T1 (Reserve stock for flash sale — ROLLBACK) | T2 (Read availability for storefront)    |
|------|----------------------------------------------|------------------------------------------|
| t1   | BEGIN                                        |                                          |
| t2   | READ(product_8.stock) = 50                   |                                          |
| t3   | product_8.stock = 0                          |                                          |
| t4   | WRITE(product_8.stock) = 0                   |                                          |
| t5   |                                              | BEGIN                                    |
| t6   |                                              | READ(product_8.stock) = 0 (DIRTY READ!)  |
| t7   |                                              | COMMIT (displays "Out of Stock")         |
| t8   | ROLLBACK                                     |                                          |
| t9   | product_8.stock reverted to 50               |                                          |

T2 committed a decision based on data that was never permanently written (stock=0 was rolled back). The product was shown as unavailable when it actually had 50 units.
...
SELECT name, price WHERE stock > 0 AND customer_id < 10
FROM product NATURAL JOIN videogame
