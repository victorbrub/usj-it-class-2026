...
**Student**: Sergio Sanmartin Aznar
**Model**: Hospital / Medical — Doctors, Rooms, Patients and Appointments
...
The following schedule involves two transactions concurrently accessing the `appointment` table of the hospital model:

- **T1**: A scheduling display system reads appointment #7's `date_time` to show the next available slot on a receptionist's screen.
- **T2**: A doctor's assistant reschedules appointment #7's `date_time` to a new time slot, but the rescheduling workflow fails and the transaction is rolled back. T1 reads the appointment date during T2's active write.
- Initial value: `appointment[id_appointment=7].date_time = '2024-06-15 10:00'`.

| Step | T1 (Scheduling display)                                                | T2 (Reschedule appointment — ROLLBACK)                     |
|------|------------------------------------------------------------------------|------------------------------------------------------------|
| t1   |                                                                        | BEGIN                                                      |
| t2   |                                                                        | WRITE(appointment[7].date_time) = '2024-06-16 14:00'       |
| t3   | BEGIN                                                                  |                                                            |
| t4   | READ(appointment[7].date_time) = '2024-06-16 14:00' (DIRTY READ!)     |                                                            |
| t5   | COMMIT (display shows incorrect rescheduled time)                      |                                                            |
| t6   |                                                                        | ROLLBACK                                                   |
| t7   |                                                                        | appointment[7].date_time reverted to '2024-06-15 10:00'    |

T1 committed a read based on data written by a transaction that was never committed. The scheduling display showed the patient an incorrect appointment time.
...
SELECT name, surnames, date_time WHERE date_time > '2024-01-01' AND id_patient < 5
FROM patient NATURAL JOIN appointment
JOIN doctor ON id_doctor = doctor.id_doctor;
