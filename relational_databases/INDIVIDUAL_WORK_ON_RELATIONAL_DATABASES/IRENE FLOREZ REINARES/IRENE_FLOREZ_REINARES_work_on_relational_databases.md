...
**Student**: Irene Florez Reinares
**Model**: Plant Shop — Clients, Plants, Care Guides and Orders
...
The following schedule involves two transactions concurrently accessing the `plant` table of the plant shop model:

- **T1**: An online customer and an in-store customer both try to purchase the last unit of plant #4 (Monstera Deliciosa). Both read the stock simultaneously, both see stock=1, and both proceed to subtract 1 and write.
- Initial value: `plant[plant_id=4].stock = 1`.

| Step | T1 (Online customer order)                    | T2 (In-store customer purchase)                |
|------|-----------------------------------------------|------------------------------------------------|
| t1   | BEGIN                                         |                                                |
| t2   | READ(plant[4].stock) = 1                      |                                                |
| t3   |                                               | BEGIN                                          |
| t4   |                                               | READ(plant[4].stock) = 1                       |
| t5   | stock = 1 - 1 = 0                             |                                                |
| t6   | WRITE(plant[4].stock) = 0                     |                                                |
| t7   | COMMIT                                        |                                                |
| t8   |                                               | stock = 1 - 1 = 0 (stale read!)                |
| t9   |                                               | WRITE(plant[4].stock) = 0                      |
| t10  |                                               | COMMIT                                         |

Final stock: 0. Both transactions sold the same unit. T1's reduction is overwritten. The plant was sold twice but there was only one in stock.
...
SELECT name, common_name, quantity WHERE price > 15 AND customer_id < 10
FROM client NATURAL JOIN order_plant
JOIN plant ON plant_id = plant.plant_id;
