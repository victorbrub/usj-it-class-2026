...
**Student**: Lucia Soria Guillen
**Model**: Music Streaming — Artists, Albums, Songs, Users and Playlists
...
The following schedule involves two transactions concurrently accessing the `song` table of the music streaming model:

- **T1**: The streaming analytics service and the radio broadcast system both increment song #22's play counter when the song is played simultaneously on two different platforms.
- **T2**: Both read the current `plays` count, add 1, and write back, producing a lost update.
- Initial value: `song[id_song=22].plays = 5000`.

| Step | T1 (Streaming analytics: +1 play)             | T2 (Radio broadcast: +1 play)                 |
|------|-----------------------------------------------|-----------------------------------------------|
| t1   | BEGIN                                         |                                               |
| t2   | READ(song[22].plays) = 5000                   |                                               |
| t3   |                                               | BEGIN                                         |
| t4   |                                               | READ(song[22].plays) = 5000                   |
| t5   | plays = 5000 + 1 = 5001                       |                                               |
| t6   | WRITE(song[22].plays) = 5001                  |                                               |
| t7   | COMMIT                                        |                                               |
| t8   |                                               | plays = 5000 + 1 = 5001 (stale read!)         |
| t9   |                                               | WRITE(song[22].plays) = 5001                  |
| t10  |                                               | COMMIT                                        |

Final count: 5001 instead of 5002. One play is permanently lost. Two events generated two commits, but only one increment was recorded.
...
SELECT username, title, plays WHERE plays > 1000 AND id_user < 5
FROM "user" NATURAL JOIN song
JOIN album ON id_artist = album.id_artist;
