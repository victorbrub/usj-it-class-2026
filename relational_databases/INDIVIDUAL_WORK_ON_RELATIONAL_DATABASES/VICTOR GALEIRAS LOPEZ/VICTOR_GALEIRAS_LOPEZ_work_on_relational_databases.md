...
**Student**: Victor Galeiras Lopez
**Model**: Gaming Platform — Players, Profiles, Games, Genres and Purchases
...
The following schedule involves two transactions concurrently inserting new rows into the `purchase` table of the gaming platform model:

- **T1**: A sales analytics dashboard counts the total purchases of game #10 to compute its bestseller rank. It counts at the start and end of an hourly report generation.
- **T2**: A new player completes the purchase of game #10 and the transaction commits between T1's two counts.
- Initial state: 350 purchases for game #10.

| Step | T1 (Analytics: count purchases of game #10)                  | T2 (Player: purchase game #10)                          |
|------|--------------------------------------------------------------|---------------------------------------------------------|
| t1   | BEGIN                                                        |                                                         |
| t2   | SELECT COUNT(*) FROM purchase WHERE game_id=10 = 350         |                                                         |
| t3   |                                                              | BEGIN                                                   |
| t4   |                                                              | INSERT INTO purchase (player_id=88, game_id=10, ...)    |
| t5   |                                                              | COMMIT                                                  |
| t6   | SELECT COUNT(*) FROM purchase WHERE game_id=10 = 351 (!!)    |                                                         |
| t7   | COMMIT (report shows inconsistent counts: 350 then 351)      |                                                         |

T1 counted the same set of rows within the same transaction and obtained different values (350 then 351) because T2 inserted a new purchase row between T1's two COUNT queries.
...
SELECT email, title, purchase_date WHERE purchase_date > '2024-01-01' AND player_id < 5
FROM player NATURAL JOIN purchase
JOIN game ON game_id = game.game_id;
