...
3. **Write** a functional example on the aplication of the model. That is, a documentation to understand the scope of aplication, what covers and what not, with examples.(0.6 points) 
...
| 1 | `BEGIN` | |
| 2 | | `BEGIN` |
| 3 | `READ(User.TotalPlaytime WHERE UserID = 1)` → 50.0 | |
| 4 | | `READ(User.TotalPlaytime WHERE UserID = 1)` → 50.0 |
| 5 | `WRITE(User.TotalPlaytime WHERE UserID = 1) = 55.0` | |
| 6 | | `WRITE(User.TotalPlaytime WHERE UserID = 1) = 53.0` |
| 7 | `COMMIT` | |
| 8 | | `COMMIT` |
...
SELECT Title, CompanyName WHERE Price > 20.00 AND AchievementID < 5 FROM Achievement NATURAL JOIN Game JOIN Developer ON Game.DevID = Developer.DevID;
...
5. Design and create a **full optimization plan** for your model. Create and `optimization.md` file explaining all the things you want to make to each table (indexing, partitioning, clustering and so on). Create a `optimize_database.sql` file that applies all those modifications in a single execution. (1.5 points)
...
6. **Create a SQL query** for your model that takes advantages from some of the optimizations you made, looking at the execution time (You may need to add more records for this to be meaningful). (0.5 points)
    * Outputs: Query and screeshots of the execution plan before and after optimizations.