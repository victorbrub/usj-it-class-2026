...
**Student**: Sergio Garcia Peiro
**Model**: Airport / Aviation — Airports, Engineers, Airplane Models, Airplanes and Tests
...
The following schedule involves two transactions concurrently accessing the `Airplane` table of the airport model:

- **T1**: An air traffic management system reads the count of airplanes currently assigned to airport 'MAD' twice: once at the start of a capacity planning report and once at the end.
- **T2**: A fleet assignment service updates an existing airplane's `airportName` from 'BCN' to 'MAD' and commits between T1's two counts.
- Initial state: 12 airplanes at airport 'MAD'.

| Step | T1 (Capacity planning report)                                    | T2 (Fleet reassignment to MAD)                        |
|------|------------------------------------------------------------------|-------------------------------------------------------|
| t1   | BEGIN                                                            |                                                       |
| t2   | SELECT COUNT(*) FROM Airplane WHERE airportName='MAD' = 12       |                                                       |
| t3   |                                                                  | BEGIN                                                 |
| t4   |                                                                  | UPDATE Airplane SET airportName='MAD' WHERE tailNum=7 |
| t5   |                                                                  | COMMIT                                                |
| t6   | SELECT COUNT(*) FROM Airplane WHERE airportName='MAD' = 13 (!!)  |                                                       |
| t7   | COMMIT (report shows inconsistent counts: 12 then 13)            |                                                       |

T1 counted the same set of rows within the same transaction and obtained different values (12 then 13) because T2 committed an update that brought a new airplane into the MAD airport between T1's two reads.
...
SELECT airportName, NIF, tailNum WHERE airportName = 'MAD' AND NIF < 10
FROM airport NATURAL JOIN engineer
JOIN airplane ON airportName = airport.airportName;
