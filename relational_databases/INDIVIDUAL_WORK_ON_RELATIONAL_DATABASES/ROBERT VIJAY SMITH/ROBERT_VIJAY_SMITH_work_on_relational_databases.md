...
**Student**: Robert Vijay Smith
**Model**: Library — Members, Books, Loans and Fines
...
The following schedule involves two transactions concurrently accessing the `fine` table of the library model:

- **T1**: The library's penalty reporting service reads fine #12's `amount` to include it in an outstanding-debt report.
- **T2**: A bookkeeping correction updates fine #12's `amount` from 8.50 to 15.00 (late overdue recalculation), but the correction process fails and the transaction is rolled back. T1 reads the fine amount during T2's active write.
- Initial value: `fine[fine_id=12].amount = 8.50`.

| Step | T1 (Penalty report)                                | T2 (Fine correction — ROLLBACK)               |
|------|-----------------------------------------------------|-----------------------------------------------|
| t1   |                                                     | BEGIN                                         |
| t2   |                                                     | WRITE(fine[12].amount) = 15.00                |
| t3   | BEGIN                                               |                                               |
| t4   | READ(fine[12].amount) = 15.00 (DIRTY READ!)         |                                               |
| t5   | COMMIT (report shows 15.00)                         |                                               |
| t6   |                                                     | ROLLBACK                                      |
| t7   |                                                     | fine[12].amount reverted to 8.50              |

T1 committed a read based on data written by a transaction that was never committed. The outstanding-debt report contains an incorrect fine amount.
...
SELECT first_name, title, return_date WHERE loan_date > '2024-01-01' AND member_id < 5
FROM member NATURAL JOIN loan
JOIN book ON isbn = loan.isbn;
