...
**Student**: Alberto Gomez Navarro
**Model**: Movie Production and Streaming — Directors, Movies, Actors and Reviews
...
The following schedule involves two transactions concurrently modifying the `movie` table of the movie production model:

- **T1**: A distribution manager increases the budget of movie #5 by 5,000,000 (extra funding approved).
- **T2**: A finance system simultaneously applies a 2,000,000 cost saving to the same movie's budget.
- Initial value: `movie_5.budget = 50,000,000`.

| Step | T1 (Extra funding: +5,000,000)            | T2 (Cost saving: -2,000,000)              |
|------|-------------------------------------------|-------------------------------------------|
| t1   | BEGIN                                     |                                           |
| t2   | READ(movie_5.budget) = 50,000,000         |                                           |
| t3   |                                           | BEGIN                                     |
| t4   |                                           | READ(movie_5.budget) = 50,000,000         |
| t5   | budget = 50,000,000 + 5,000,000           |                                           |
| t6   | WRITE(movie_5.budget) = 55,000,000        |                                           |
| t7   | COMMIT                                    |                                           |
| t8   |                                           | budget = 50,000,000 - 2,000,000 = 48,000,000 |
| t9   |                                           | WRITE(movie_5.budget) = 48,000,000        |
| t10  |                                           | COMMIT                                    |

Final value: 48,000,000. Correct value should be 53,000,000 (55M - 2M). T1's update is lost.
...
SELECT title, role_name WHERE release_year > 2010 AND budget > 1000000
FROM movie NATURAL JOIN actor
JOIN casting ON actor_id = casting.actor_id;
