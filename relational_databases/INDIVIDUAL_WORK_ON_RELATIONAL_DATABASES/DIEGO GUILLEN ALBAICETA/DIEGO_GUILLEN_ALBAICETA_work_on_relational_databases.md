...
**Student**: Diego Guillen Albaiceta
**Model**: Plushie Shop — Suppliers, Plushies, Clients, Orders and Shops
...
The following schedule involves two transactions concurrently modifying the `plushie` table of the plushie shop model:

- **T1**: Customer A's order places a request for 3 units of plushie #5 (Totoro). Reads stock, subtracts 3 and writes.
- **T2**: Customer B's order simultaneously requests 2 units of the same plushie #5. Also reads the original stock and subtracts 2.
- Initial value: `plushie[plushie_id=5].stock_quantity = 3`.

| Step | T1 (Customer A orders 3 units)                       | T2 (Customer B orders 2 units)                        |
|------|------------------------------------------------------|-------------------------------------------------------|
| t1   | BEGIN                                                |                                                       |
| t2   | READ(plushie[5].stock_quantity) = 3                  |                                                       |
| t3   |                                                      | BEGIN                                                 |
| t4   |                                                      | READ(plushie[5].stock_quantity) = 3                   |
| t5   | stock_quantity = 3 - 3 = 0                           |                                                       |
| t6   | WRITE(plushie[5].stock_quantity) = 0                 |                                                       |
| t7   | COMMIT                                               |                                                       |
| t8   |                                                      | stock_quantity = 3 - 2 = 1 (stale read!)              |
| t9   |                                                      | WRITE(plushie[5].stock_quantity) = 1                  |
| t10  |                                                      | COMMIT                                                |

Final stock: 1. True stock: 0 (all 3 sold). T1's reduction is overwritten by T2's stale calculation. The system shows 1 unit available but none actually remain.
...
SELECT client_name, plushie_type, quantity WHERE price > 20 AND client_id < 10
FROM client NATURAL JOIN order_plushie
JOIN plushie ON plushie_id = plushie.plushie_id;
