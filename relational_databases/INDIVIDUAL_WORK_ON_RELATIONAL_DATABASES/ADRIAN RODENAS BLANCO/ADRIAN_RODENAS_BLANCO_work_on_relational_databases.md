...
**Student**: Adrian Rodenas Blanco
**Model**: Valorant — Agents, Players, Matches and Rounds
...
The following schedule involves two transactions concurrently accessing the `Participacion` table of the Valorant model:

- **T1**: A statistics module reads the kill count for player #5 in match #10 at two points during the same analysis report.
- **T2**: A match moderator corrects a kill registration error for the same record: updates Kills from 15 to 12 and commits between T1's two reads.
- Initial value: `Participacion[ID_Jugador=5, ID_Partida=10].Kills = 15`.

| Step | T1 (Statistics report)                                    | T2 (Moderator correction)                             |
|------|-----------------------------------------------------------|-------------------------------------------------------|
| t1   | BEGIN                                                     |                                                       |
| t2   | READ(Participacion[5,10].Kills) = 15                      |                                                       |
| t3   |                                                           | BEGIN                                                 |
| t4   |                                                           | READ(Participacion[5,10].Kills) = 15                  |
| t5   |                                                           | Kills = 12 (corrected value)                          |
| t6   |                                                           | WRITE(Participacion[5,10].Kills) = 12                 |
| t7   |                                                           | COMMIT                                                |
| t8   | READ(Participacion[5,10].Kills) = 12 (DIFFERENT VALUE!)   |                                                       |
| t9   | COMMIT                                                    |                                                       |
...
SELECT Nombre_Usuario, Kills WHERE Kills > 10 AND ID_Partida < 50
FROM Jugador NATURAL JOIN Participacion
JOIN Partida ON ID_Jugador = Partida.ID_Partida;
