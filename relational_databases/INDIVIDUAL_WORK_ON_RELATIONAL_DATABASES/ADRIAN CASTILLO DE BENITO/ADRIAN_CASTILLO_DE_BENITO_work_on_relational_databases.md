...
**Student**: Adrian Castillo de Benito
**Model**: APP_DIET — Diet and Nutrition Management System
...
The following schedule involves two transactions concurrently modifying the `diet` table of the APP_DIET model:

- **T1**: A nutritionist adds a morning snack to diet plan #3, increasing `daily_calories` by 200 kcal.
- **T2**: Another nutritionist removes an afternoon snack from the same diet plan #3, decreasing `daily_calories` by 100 kcal.
- Initial value: `diet_3.daily_calories = 2000` kcal.

| Step | T1 (Add morning snack: +200 kcal)    | T2 (Remove afternoon snack: -100 kcal) |
|------|--------------------------------------|----------------------------------------|
| t1   | BEGIN                                |                                        |
| t2   | READ(diet_3.daily_calories) = 2000   |                                        |
| t3   |                                      | BEGIN                                  |
| t4   |                                      | READ(diet_3.daily_calories) = 2000     |
| t5   | daily_calories = 2000 + 200 = 2200   |                                        |
| t6   | WRITE(diet_3.daily_calories) = 2200  |                                        |
| t7   | COMMIT                               |                                        |
| t8   |                                      | daily_calories = 2000 - 100 = 1900     |
| t9   |                                      | WRITE(diet_3.daily_calories) = 1900    |
| t10  |                                      | COMMIT                                 |
...
SELECT name, email WHERE imc > 25 AND id_diet < 10
FROM user NATURAL JOIN recommendation
JOIN diet ON id_diet = diet.id_diet;
