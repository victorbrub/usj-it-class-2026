...
**Student:** MANUEL MARTÍNEZ MERINO
...
Solid work overall with good coverage of concurrency and query optimization. The access verification section lacks visual evidence, which is required to validate the correct behaviour of the defined roles. The optimization plan omits partitioning as a strategy, and the explanation for the sequential scan behaviour is not technically correct. Review the relationship between query structure and index applicability.
---
...
**Score: 0.0 / 0.3** - 0 screenshot(s) found, no evidence. Access verification present but denied scenario not clearly shown.
...
**Score: 1.0 / 1.5** - Missing Partitioning strategy. This is the first thing to do when optimizing a database.

### Exercise 4.6 - Query Demonstrating Optimization Benefit (0.5 points)

**Score: 0.0 / 0.5** - Explanation not correct. If there is a low record issue, you should add more. If not using index scan, is likely that the indexing approach does not cover your query filtering/joining scenario, so the query is not fitted to your optimizations.
