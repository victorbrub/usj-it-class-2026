...
The submission is functional but skips key conceptual steps. The access control section jumps directly into implementation without first defining the user roles and their data access scope, which is a required prerequisite. The optimization plan is limited in scope, omits partitioning entirely, and the accompanying analysis provides no comparison of execution plans before and after the changes. Justification for each optimization decision must be explicit.
---
...
**Score: 0.0 / 0.3** - No definition per se of the types of users and what data can each user see. You need to define this before implementing it.
...
**Score: 0.5 / 1.5** - Poor optimization plan. Missing partitioning approach, and lacks specific implementations and its motivation.

### Exercise 4.6 - Query Demonstrating Optimization Benefit (0.5 points)

**Score: 0.2 / 0.5** - No explanation on the improvement or comparison of the execution plans.
