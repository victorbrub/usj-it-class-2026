...
**Student**: Adrian Liso Cebrian
**Model**: Football — Clubs, Players, Stadiums and Trophies
...
The following schedule involves two transactions concurrently accessing the `player` table of the football model:

- **T1**: A transfer agent updates player '12345678A' to team #7 to test a deal, but the negotiation fails and the transaction is rolled back.
- **T2**: A journalist reads the same player's current team to publish a transfer news article.
- Initial value: `player['12345678A'].team_id = 3`.

| Step | T1 (Player transfer — rolled back)              | T2 (Read player for news article)                     |
|------|------------------------------------------------|-------------------------------------------------------|
| t1   | BEGIN                                          |                                                       |
| t2   | READ(player['12345678A'].team_id) = 3          |                                                       |
| t3   | player['12345678A'].team_id = 7                |                                                       |
| t4   | WRITE(player['12345678A'].team_id) = 7         |                                                       |
| t5   |                                                | BEGIN                                                 |
| t6   |                                                | READ(player['12345678A'].team_id) = 7                 |
| t7   |                                                | COMMIT (article written: "player transferred to team #7") |
| t8   | ROLLBACK                                       |                                                       |
| t9   | player['12345678A'].team_id reverted to 3      |                                                       |
...
SELECT player_name, birth_date WHERE age < 25 AND team_id < 3
FROM player NATURAL JOIN team
JOIN stadium ON player.team_id = stadium.stadium_id;
