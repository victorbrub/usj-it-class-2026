...
# Group Work: Data Warehouse Architecture Design — Scenario 05: EduTrack: Online Learning Platform
...
### Scenario 05 — EduTrack: Online Learning Platform

**Domain**: A platform offering video-based courses across technology, business, and creative subjects. Students enrol in courses, complete lessons, take quizzes, and earn certificates. Instructors create and maintain course content.

**Analytical questions**:
- Which courses and categories have the highest completion rates and which have the highest drop-off?
- How does course pricing correlate with enrolment volume and completion?
- Which instructors produce the highest-rated and best-completing courses?
- What is the average time from enrolment to certificate for each course category?
- How effective are promotional discount codes at driving new vs. returning learner enrolments?

**PostgreSQL core entities**: courses, course sections, lessons, instructors, categories, students, enrolments, lesson completions, quiz attempts, quiz questions, certificates, promotional codes.

**MongoDB enrichment**: Enrich the course catalog with simulated external review and metadata documents in the style of Coursera or Udemy public pages. Each document covers one course and includes: external rating, number of external ratings, skill tags (array), prerequisite descriptions, last updated date, and language.

**Redis signal**: Sorted set of currently active learner counts per course. Score is the number of students who completed a lesson in the last 24 hours. Updated by the lesson completion event handler.

**SCD challenge**: A course undergoes major version updates (content revision), which may change its difficulty rating or category assignment. An instructor may move from one organisation to another. Design your dimensions to track the course version at time of enrolment and the completion at time it occurred.

**Suggested analytical focus**: enrolment fact (one row per student-course enrolment), lesson completion fact (one row per lesson completion event), course dimension, instructor dimension, student dimension, date dimension, category dimension.
