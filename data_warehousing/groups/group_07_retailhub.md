...
# Group Work: Data Warehouse Architecture Design — Scenario 07: RetailHub: E-commerce Marketplace
...
### Scenario 07 — RetailHub: E-commerce Marketplace

**Domain**: An online marketplace where independent sellers list products and buyers place orders. The platform handles payments, coordinates fulfilment, manages returns, and runs promotional campaigns.

**Analytical questions**:
- Which product categories, sellers, and price bands generate the most revenue and gross margin?
- What is the return rate by product category and seller, and what are the most common return reasons?
- How do promotional campaigns and discount codes affect average order value and conversion?
- Which sellers are growing their GMV (gross merchandise value) and which are declining?
- What is the relationship between product review score and subsequent sales velocity?

**PostgreSQL core entities**: sellers, products, product categories, buyers, orders, order items, payments, returns, promotions, promotion applications, product reviews.

**MongoDB enrichment**: Enrich products in the electronics category with technical specification metadata. Use a mock dataset modelled on the **Open Food Facts API** structure (https://world.openfoodfacts.org/data — free, no auth) for food products, or design a mock specification document for electronics with fields such as brand, model number, weight, dimensions, compatibility tags (array), and energy rating.

**Redis signal**: Hash per product storing real-time inventory state: `stock_count`, `reserved_count` (in active carts), `last_restocked_date`, and `restock_threshold`. Written by the warehouse management system.

**SCD challenge**: Product prices change due to promotions, market adjustments, or seller decisions. A seller's performance tier (Standard / Premium / Verified) changes based on fulfilment metrics. Design dimensions to preserve the price at time of purchase and the seller tier at time of the order.

**Suggested analytical focus**: order line fact (one row per product per order), return fact (one row per return request), product dimension, seller dimension, buyer dimension, category dimension, date dimension, promotion dimension.
