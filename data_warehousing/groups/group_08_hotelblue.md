...
# Group Work: Data Warehouse Architecture Design — Scenario 08: HotelBlue: Hotel Chain Reservation System
...
### Scenario 08 — HotelBlue: Hotel Chain Reservation System

**Domain**: A hotel chain with properties in multiple cities. Guests book rooms of various types, consume in-hotel services (restaurant, spa, parking), participate in a loyalty programme, and leave reviews.

**Analytical questions**:
- Which properties, room types, and booking channels generate the most revenue per available room (RevPAR)?
- How does booking lead time correlate with room rate and occupancy?
- What is the contribution of ancillary services to total guest spend by property and season?
- How effective is the loyalty programme at increasing repeat stay rates?
- Which source markets (guest countries) are growing and which are declining per property?

**PostgreSQL core entities**: properties, room types, rooms, guests, reservations, reservation rooms, services (restaurant/spa/parking), service consumptions, loyalty accounts, booking channels, invoices.

**MongoDB enrichment**: Enrich each property with aggregated external review sentiment. Use a simulated dataset modelled on TripAdvisor-style data: each document covers one property and includes an overall sentiment score, scores per dimension (cleanliness, service, location, value), a summary of most mentioned positive and negative topics (arrays of strings), and the source review count.

**Redis signal**: Hash per property with current occupancy state: `rooms_available`, `rooms_occupied`, `rooms_cleaning`, `checkins_today`, `checkouts_today`. Updated by the property management system on each check-in and check-out event.

**SCD challenge**: Room rates change seasonally and with demand. A guest's loyalty tier (Silver / Gold / Platinum) changes when point thresholds are crossed. Design dimensions to track the rate paid at reservation time and the loyalty tier at time of stay.

**Suggested analytical focus**: reservation fact (one row per room night stayed), service consumption fact (one row per service charge posted to a guest bill), property dimension, room type dimension, guest dimension, date dimension, booking channel dimension, loyalty tier dimension.
