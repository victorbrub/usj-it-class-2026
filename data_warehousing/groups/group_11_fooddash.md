...
# Group Work: Data Warehouse Architecture Design — Scenario 11: FoodDash: Food Delivery Platform
...
### Scenario 11 — FoodDash: Food Delivery Platform

**Domain**: A food delivery marketplace. Customers order from restaurants through an app. Delivery drivers pick up and deliver orders. Restaurants manage their menu, accept orders, and are rated by customers.

**Analytical questions**:
- Which restaurants, cuisine types, and price bands have the highest order volume and revenue?
- How does estimated vs. actual delivery time vary by zone, time of day, and day of week?
- What is the relationship between restaurant hygiene score and order volume?
- Which drivers deliver the fastest and have the fewest complaints?
- What fraction of orders are refunded or disputed, and what are the most common reasons?

**PostgreSQL core entities**: restaurants, cuisine types, menu items, menu item categories, customers, drivers, zones, orders, order items, deliveries, restaurant ratings, driver ratings, refunds.

**MongoDB enrichment**: Enrich restaurants with public health inspection data. Use a simulated dataset modelled on datasets such as the **UK Food Hygiene Rating Scheme** (https://ratings.food.gov.uk/open-data — open data, no auth needed). Each document covers one restaurant with its hygiene rating (0–5), last inspection date, inspection authority, issues found (array), and address.

**Redis signal**: Sorted set of currently active restaurants ranked by estimated delivery time. Score is the current estimated preparation + delivery time in minutes. Hash per driver storing current status (`available`, `en_route_pickup`, `en_route_delivery`, `offline`) and GPS zone ID. Both are updated in near-real-time by the dispatch system.

**SCD challenge**: Menu item prices change frequently. A restaurant's cuisine classification can be updated by the operator. A driver's assigned zone changes based on shift scheduling. Design dimensions to preserve menu prices at order time and restaurant attributes at the time of each order.

**Suggested analytical focus**: order line fact (one row per menu item per order), delivery performance fact (one row per delivery), restaurant dimension, driver dimension, customer dimension, zone dimension, date dimension, cuisine dimension.
