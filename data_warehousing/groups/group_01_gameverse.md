...
# Group Work: Data Warehouse Architecture Design — Scenario 01: GameVerse: Video Game Digital Distribution
...
### Scenario 01 — GameVerse: Video Game Digital Distribution

**Domain**: A digital game store. Users purchase and play games, write reviews, and unlock achievements. Publishers and developers publish titles across multiple platforms.

**Analytical questions**:
- Which genres and platforms drive the most revenue?
- How does a game's external critical score correlate with actual purchase volume?
- Which publishers are growing and which are in decline?
- What is the relationship between achievement design and user engagement?
- Which users are at churn risk based on purchase and activity patterns?

**PostgreSQL core entities**: games, publishers, developers, genres, platforms, users, purchases (user library), reviews, achievements, user achievement unlocks.

**MongoDB enrichment**: Enrich the game catalog with critic and community metadata from the **RAWG Video Games Database API** (https://rawg.io/apidocs — free tier). Each document enriches one game with external tags, ESRB rating, community wishlist/ownership counts, and a short description.

**Redis signal**: Sorted set of daily trending game scores. Scores are computed from purchase velocity over a rolling window. Updated hourly by an application job.

**SCD challenge**: Game prices change over promotional periods. A developer studio can be acquired by a publisher. Design your dimensions to preserve the state at the time of each transaction.

**Suggested analytical focus**: transactional fact (purchases), engagement snapshot fact (monthly hours played per user per game), game dimension, user dimension, publisher/developer dimension, platform dimension, date dimension.
