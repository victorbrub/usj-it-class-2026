...
# Group Work: Data Warehouse Architecture Design — Scenario 02: StreamSound: Music Streaming Platform
...
### Scenario 02 — StreamSound: Music Streaming Platform

**Domain**: A music streaming service. Subscribers listen to tracks, follow artists, create and share playlists, and can upgrade to a paid tier. Rights holders (labels) license catalogues to the platform.

**Analytical questions**:
- Which genres, artists, and tracks drive the most listening time?
- How does subscription tier (free vs. premium) affect listening behaviour and track skips?
- Which artists are growing their listener base and which are declining?
- How effective are curated playlists at driving discovery of new tracks?
- What is the average user lifetime and what predicts early churn?

**PostgreSQL core entities**: tracks, albums, artists, genres, labels (rights holders), users, subscriptions, listening events (stream records), playlists, playlist tracks.

**MongoDB enrichment**: Enrich the track and artist catalog with audio features and popularity metadata. Use the **MusicBrainz API** (https://musicbrainz.org/doc/MusicBrainz_API — free, no auth) or a simulated document with fields such as tempo, energy, danceability, valence, key, loudness, and external play count.

**Redis signal**: Hash per user storing real-time session state: current track playing, session start time, tracks skipped in session, and device type. Written by the streaming backend on every play event.

**SCD challenge**: An artist's genre classification can change over time (a pop artist transitions to R&B). A user's subscription tier changes when they upgrade or downgrade. Design dimensions to reflect the state at the time of each listening event.

**Suggested analytical focus**: stream event fact (one row per track play), playlist engagement fact (one row per playlist session), track dimension, artist dimension, user dimension, date dimension, label dimension.
