...
# Group Work: Data Warehouse Architecture Design — Scenario 10: CinemaPlus: Movie Theater Chain
...
### Scenario 10 — CinemaPlus: Movie Theater Chain

**Domain**: A cinema chain with multiplexes in several cities. Customers book tickets for film showings in different screen formats (standard, IMAX, 4DX). Concession purchases are tracked. Members earn points through a loyalty programme.

**Analytical questions**:
- Which films, genres, and screen formats generate the most ticket revenue per screening?
- How does booking lead time and channel (app, web, walk-in) correlate with seat fill rate?
- What is the contribution of concessions to total spend per customer visit?
- Which demographics (age group, city) show the highest frequency of visits?
- How does a film's external critic score correlate with actual audience attendance?

**PostgreSQL core entities**: cinemas, screens, screen formats, films, showings (a scheduled screening of a film), seats, customers, bookings, booking seats, concession items, concession purchases, loyalty accounts.

**MongoDB enrichment**: Enrich the film catalog with metadata from the **OMDb API** (http://www.omdbapi.com — free tier with registration, 1,000 requests/day) or the **TMDB API** (https://developer.themoviedb.org — free with registration). Each document enriches one film with genre tags (array), director, cast (array), production country, runtime, Metascore, IMDb rating, and a short plot summary.

**Redis signal**: Sorted set per cinema of currently trending films. Score is calculated from tickets sold in the last 48 hours at that cinema. Updated on every booking confirmation.

**SCD challenge**: A film's classification (age rating) can be changed after initial release. A customer's loyalty tier changes when point thresholds are crossed. Design dimensions to reflect the classification at time of booking and the loyalty tier at time of visit.

**Suggested analytical focus**: ticket booking fact (one row per seat booked per showing), concession purchase fact (one row per item purchased per visit), film dimension, cinema dimension, screen dimension, customer dimension, date dimension, format dimension.
