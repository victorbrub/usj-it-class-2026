...
# Group Work: Data Warehouse Architecture Design — Scenario 09: FinTrack: Personal Finance Application
...
### Scenario 09 — FinTrack: Personal Finance Application

**Domain**: A personal finance app that aggregates bank account data. Users connect bank accounts, track transactions, assign categories to spending, set monthly budgets, and define savings goals.

**Analytical questions**:
- Which spending categories account for the largest share of user expenditure, and how does this vary by demographic?
- How accurately do users forecast their monthly spending (budgeted vs. actual)?
- What is the average time to reach a savings goal, and which goal types are most commonly abandoned?
- Which merchant types are associated with the highest average transaction value?
- How does premium subscription status correlate with budget adherence and savings goal completion?

**PostgreSQL core entities**: users, bank accounts, account types, transactions, spending categories, merchant records, budgets, budget lines, savings goals, savings contributions, subscriptions.

**MongoDB enrichment**: Enrich merchants with standardised classification metadata. Use a mock dataset based on ISO 18245 **Merchant Category Codes (MCC)**: each document covers one merchant and includes its MCC code, MCC description, merchant name, address, city, country, and a tags array (e.g., `["food", "delivery", "subscription"]`).

**Redis signal**: Hash per user storing real-time budget alert state: `budget_used_pct` per active budget category, and a `alert_triggered` boolean flag. Written nightly by a batch budget evaluation job and reset at the start of each month.

**SCD challenge**: A user's spending category assignment for a recurring merchant can change (e.g., Netflix reclassified from Entertainment to Subscriptions). A user's subscription plan changes over time. Design dimensions to reflect the category as assigned at the time of the transaction and the subscription tier at month-end snapshot.

**Suggested analytical focus**: transaction fact (one row per bank transaction), budget snapshot fact (monthly snapshot of budgeted vs. actual per category per user), user dimension, merchant dimension, spending category dimension, account dimension, date dimension.
