...
# Group Work: Data Warehouse Architecture Design — Scenario 12: SportStats: Professional Sports League
...
### Scenario 12 — SportStats: Professional Sports League

**Domain**: A professional football (soccer) league management system. Teams compete in a season with home and away fixtures. Player statistics are recorded per match. Players can transfer between clubs during transfer windows.

**Analytical questions**:
- Which teams and players contribute most to goals, assists, and defensive metrics over a season?
- How does home vs. away performance vary by team and stadium?
- What is the relationship between squad market value and league position at season end?
- How do player transfer fees correlate with subsequent on-pitch performance?
- Which age groups and nationalities are most represented and perform best in the league?

**PostgreSQL core entities**: teams, stadiums, seasons, matches, match events (goal, card, substitution), players, player contracts (club membership periods), positions, transfers, standings (weekly snapshot per team).

**MongoDB enrichment**: Enrich player profiles with biographical and career history metadata. Use the **API-Football** free tier (https://www.api-football.com — 100 calls/day free) or a mock dataset. Each document covers one player with nationality, date of birth, height, weight, preferred foot, career clubs history (array of objects with club and season), and international caps.

**Redis signal**: Sorted set of current league standings by points. Score is the team's total points. Updated after each match result is confirmed. A second sorted set ranks top scorers by goals.

**SCD challenge**: A player transfers to a different team during the season. A team can be promoted or relegated between seasons, changing its division. Design dimensions so that player statistics are always linked to the correct club at the time of the match, not the current club.

**Suggested analytical focus**: match event fact (one row per in-match event: goal, card, etc.), season performance snapshot fact (one row per player per season), player dimension, team dimension, match dimension, stadium dimension, date dimension, position dimension.
