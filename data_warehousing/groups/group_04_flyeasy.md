...
# Group Work: Data Warehouse Architecture Design — Scenario 04: FlyEasy: Airline Booking Platform
...
### Scenario 04 — FlyEasy: Airline Booking Platform

**Domain**: A regional airline with direct and connecting flights. Passengers book seats in different fare classes, accumulate loyalty points, and can check in baggage and select ancillary services.

**Analytical questions**:
- Which routes are most profitable and which are consistently underbooked?
- How does fare class mix (economy/business) vary by route, season, and booking lead time?
- How effective is the loyalty programme at increasing repeat booking rates?
- What is the on-time performance by route, aircraft type, and time of year?
- How do ancillary services (bags, seat upgrades) contribute to revenue per passenger?

**PostgreSQL core entities**: airports, routes, aircraft types, flights (scheduled instances of a route), fare classes, passengers, bookings, booking segments, seats, loyalty accounts, ancillary services.

**MongoDB enrichment**: Enrich airport and route data with environmental and contextual metadata. Use the **Open-Meteo API** (https://open-meteo.com — free, no auth) to simulate historical weather conditions at departure airports, stored as documents with fields for temperature, wind speed, precipitation, and visibility at scheduled departure time.

**Redis signal**: Hash per flight storing real-time operational state: current status (on-time / delayed / cancelled), remaining seat count by fare class, and gate number. Updated by the operations system as events occur.

**SCD challenge**: Fare prices on a route change continuously by yield management. A passenger's loyalty tier (Bronze / Silver / Gold) changes when they reach point thresholds. Design dimensions to reflect the fare at time of booking and the loyalty tier at time of travel.

**Suggested analytical focus**: booking fact (one row per passenger segment booked), flight performance fact (one row per flight operated), route dimension, passenger dimension, aircraft dimension, airport dimension, date dimension, fare class dimension.
