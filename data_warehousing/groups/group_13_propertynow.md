...
# Group Work: Data Warehouse Architecture Design — Scenario 13: PropertyNow: Real Estate Listing Platform
...
### Scenario 13 — PropertyNow: Real Estate Listing Platform

**Domain**: An online real estate platform where agents and agencies list properties for sale or rent. Buyers and tenants browse listings, schedule viewings, and submit offers. The platform tracks the full property lifecycle from listing to closing.

**Analytical questions**:
- How long does a property stay on the market before receiving an accepted offer, and what factors predict faster sales?
- Which agents and agencies close the most transactions and at what average price premium over asking?
- How do neighbourhood characteristics (school ratings, walkability, transport links) correlate with price per square metre?
- What is the seasonal pattern in new listings, viewings, and closings by property type and city?
- Which property types and price bands have the highest viewing-to-offer conversion rate?

**PostgreSQL core entities**: agencies, agents, properties, property types, listings, listing price histories, viewings, offers, contracts, buyers, sellers (owners), neighbourhoods.

**MongoDB enrichment**: Enrich each neighbourhood with contextual quality-of-life data. Use a simulated dataset modelled on Walk Score or similar open data, with fields for walkability score, transit score, school rating (nearest primary and secondary), average commute time to city centre, green space area, and a tags array (e.g., `["family-friendly", "nightlife", "quiet"]`).

**Redis signal**: Sorted set of currently most-viewed listings. Score is the number of page views in the last 24 hours. Updated on every listing page view event. A separate key per listing stores a counter of saved-to-favourites actions.

**SCD challenge**: A listing's asking price changes one or more times before an offer is accepted (price reductions are common). An agent can move from one agency to another. Design dimensions to track the price at the time of each viewing and the agent's agency at the time of the transaction.

**Suggested analytical focus**: viewing fact (one row per scheduled property viewing), transaction fact (one row per completed sale or rental contract), property dimension, agent dimension, neighbourhood dimension, buyer/seller dimension, date dimension, property type dimension.
