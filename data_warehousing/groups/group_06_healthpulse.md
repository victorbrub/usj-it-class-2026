...
# Group Work: Data Warehouse Architecture Design — Scenario 06: HealthPulse: Clinic Management System
...
### Scenario 06 — HealthPulse: Clinic Management System

**Domain**: A network of private clinics. Patients register, book appointments with doctors in specific specialisations, receive diagnoses, are prescribed medication, and are invoiced for services. The system operates across multiple clinic locations.

**Analytical questions**:
- Which specialisations and clinic locations have the highest appointment volume and revenue?
- What is the average time from appointment booking to attendance, and how does it vary by specialisation?
- How does diagnosis frequency vary by patient age group, gender, and location?
- Which prescribed medications are most common by diagnosis category?
- What percentage of invoices are settled within 30 days, and which patient segments have the highest outstanding debt?

**PostgreSQL core entities**: clinics, doctors, specialisations, patients, appointments, diagnoses, prescriptions, medications, invoices, invoice lines, insurance providers, insurance coverages.

**MongoDB enrichment**: Enrich diagnoses with standardised clinical terminology from the **ICD-10 code set**. Use the **WHO ICD API** (https://icd.who.int/icdapi — free registration required) or a locally stored ICD-10 lookup file. Each document represents one ICD-10 code with its full description, chapter, block, and parent code hierarchy.

**Redis signal**: Sorted set of current appointment queue length per clinic. Score is the number of patients currently checked in and waiting. Updated in real time by the reception desk system.

**SCD challenge**: A patient's insurance coverage changes when they change insurer or plan. A doctor may change their primary specialisation or move to a different clinic. Design dimensions to reflect the insurance plan at the time of the appointment and the doctor's clinic assignment at the time of the visit.

**Suggested analytical focus**: appointment fact (one row per appointment attended), prescription fact (one row per prescription issued), patient dimension, doctor dimension, clinic dimension, diagnosis dimension, date dimension, insurance dimension.
