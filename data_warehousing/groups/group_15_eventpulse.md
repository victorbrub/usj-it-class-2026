...
# Group Work: Data Warehouse Architecture Design — Scenario 15: EventPulse: Event Ticketing Platform
...
### Scenario 15 — EventPulse: Event Ticketing Platform

**Domain**: An event ticketing marketplace. Promoters publish events (concerts, sports, theatre, festivals) at venues. Customers buy tickets in different categories (standing, seated, VIP). The platform supports primary sales and a controlled resale market.

**Analytical questions**:
- Which event categories, venues, and artist types generate the most ticket revenue?
- How does ticket pricing (face value vs. resale premium) vary by event type, seat category, and booking lead time?
- What fraction of events sell out and how does time-to-sellout correlate with artist popularity and venue capacity?
- How effective are promotional presales (fan club, credit card partners) at driving early sales?
- What is the resale market size relative to primary sales, and which events attract the most secondary market activity?

**PostgreSQL core entities**: venues, venue zones (sections/stands), events, event categories, artists/performers, event lineups (artist-event link), promoters, customers, orders, tickets, ticket categories, presale campaigns, resale listings, resale transactions.

**MongoDB enrichment**: Enrich artist and event profiles with metadata in the style of **Songkick** or **Bandsintown**. Use a simulated dataset where each document covers one artist with genre tags (array), followers count, home country, top tracks (array), social media handles, and upcoming event count from external sources.

**Redis signal**: Sorted set per event of ticket category availability. Score for each ticket category key is the number of tickets remaining. Updated on every purchase confirmation. A separate counter key tracks real-time demand spikes (page views per minute per event).

**SCD challenge**: Ticket face value prices can be adjusted by the promoter before the event (dynamic pricing tiers). A venue's maximum capacity can change if sections are reserved for production or safety reasons. Design dimensions to reflect the price tier at the time of purchase and the available capacity at time of sale.
