...
# Group Work: Data Warehouse Architecture Design — Scenario 14: CarShare: Urban Vehicle Sharing Platform
...
### Scenario 14 — CarShare: Urban Vehicle Sharing Platform

**Domain**: An urban vehicle sharing service. Members book electric cars, scooters, and bicycles by the minute or hour from self-service stations. The platform manages vehicle availability, trip billing, charging infrastructure, and incidents.

**Analytical questions**:
- Which vehicle types, zones, and time slots generate the most revenue and have the highest utilisation rate?
- What is the average trip duration and distance by vehicle type, user segment, and zone?
- How does battery/charge level at trip start correlate with trip abandonment or charging events mid-trip?
- Which stations are consistently overdemanded (empty vehicles) or underdemanded (full vehicles)?
- What is the incident rate by vehicle type, zone, and driver age group?

**PostgreSQL core entities**: vehicle types, vehicles, zones, stations, members, membership plans, trips, trip billing records, charging events, incidents, incident categories, maintenance records.

**MongoDB enrichment**: Enrich vehicle models with manufacturer specification metadata. Use a simulated dataset modelled on the **Electric Vehicle Database** (https://ev-database.org — public data). Each document covers one vehicle model with battery capacity (kWh), real-world range (km), charging speed (kW AC/DC), dimensions, cargo volume, and a features array (e.g., `["heated_seats", "autopilot", "fast_charge"]`).

**Redis signal**: Hash per station storing real-time availability: `vehicles_available` (count), `docks_free` (count), `last_updated` timestamp. Sorted set per zone ranking stations by vehicles available. Both updated by on-board vehicle telematics at end of each trip.

**SCD challenge**: A member's pricing plan changes when they upgrade (e.g., from Pay-as-you-go to Monthly Unlimited). Vehicle condition tier (New / Good / Fair / Maintenance) degrades over time and is updated after each maintenance check. Design dimensions to apply the correct pricing plan and vehicle tier to each trip.

**Suggested analytical focus**: trip fact (one row per trip), charging event fact (one row per charging session), vehicle dimension, member dimension, station dimension, zone dimension, date dimension, vehicle type dimension.
